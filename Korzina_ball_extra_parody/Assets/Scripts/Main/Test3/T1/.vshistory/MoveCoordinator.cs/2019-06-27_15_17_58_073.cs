    ...
    class MoveCoordinator : MonoBehaviour
    {
        ...
        public static int P
        {
            ...
            set
            {
                if(value - p>0)
                total += value - p;
                _instance.totalText.text = "Total: " + total;
                p = value;
        ...
        public static void AddForceToBall()
        {
            ...
            rb2D.AddForce(Vector3.up * 0.05f);
            //var r = UnityEngine.Random.Range(0, 2) == 0;
            if (CollisionKorzina.IsActiveLeft)
        ...
        void Update()
        {
            ...
            var p = korzina.transform.position;
            p.x = item.position.x;
            korzina.transform.position = p;

            if (Math.Abs(item.position.y - korzina_y) < 0.7f)
            {
