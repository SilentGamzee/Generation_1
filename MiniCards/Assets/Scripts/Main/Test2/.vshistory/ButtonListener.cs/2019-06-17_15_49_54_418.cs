...
namespace Assets.Scripts.Main
{
    class ButtonListener : MonoBehaviour
    {
        ...
        public static int life_count = 2;
        private static List<GameObject> clicked = new List<GameObject>();
        ...
        private static IEnumerator OnLose(GameObject card)
        {
            life_count--;
            _instance.lifeText.text = "Life: " + life_count;
            yield return new WaitForSeconds(1f);

            InitScript.HideImage(card);
            clicked.Remove(card);
        }
        ...
        public static void OnCardClick(GameObject card)
        {
            ...
            if (clicked.Contains(card)) return;
            clicked.Add(card);
            ...
            else
            {
                ...
                {
                    if (life_count > 0)
                        _instance.StartCoroutine(OnLose(card));
                    else
                        PlayerManager.Lose();
                }
                else
