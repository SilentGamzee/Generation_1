...
namespace Assets.Scripts.Main
{
    class MoveCoordinator : MonoBehaviour
    {
        ...
        public static void Reset()
        {
            var trans_row = InitScript._instance.randomRow.transform;
            for (var i = 0; i < trans_row.childCount; i++)
                objPoints_x[trans_row.GetChild(i).gameObject] = i;
        }
        ...
        void UpdateColPos()
        {
            ...
            if (IsMovingDown)
                ...
                target_y = movePoint.y;

            item.position = Vector3.MoveTowards(pos, new Vector3(pos.x, target_y, pos.z), step);
            if (Math.Abs(pos.y - target_y) <= 0.1f)
            {
                if (!IsMovingDown)
                {
